...
\subsection{Limitless RDD with Historical Data: The Hurricaine \maria\
...
\section{Conventional RDD Methods}


\subsection{The \textsc{Ancova} Model for RDDs}\label{sec:robust-analys-covar}

The classical analysis of covariance (\textsc{ancova}) model says that for
$i=1,\ldots, k$ and $j=1, \ldots, n_{i}$,
$Y_{ij} = \alpha_{i} + \beta X_{ij} + \epsilon_{ij}$, where $\epsilon_{ij}
\sim \mathrm{Normal}(0, \sigma^{2})$ is independent of the continuous
covariate $X_{ij}$; in the classical development of RDDs, the $k=2$
\textsc{ancova} is a leading option among statistical models
\citep{thistlethwaite1960regression}.
% , a study of effects of public recognition of scholastic
% achievement on high school students' attitudes toward intellectualism
% and plans for postgraduate education. This RDD compared finalists
% of the 1957 National Merit Scholarship competition to other students
% who had participated but scored just below the qualifying
% test's cutpoint for advancement to the final round. To isolate effects of recognition
% from effects of the scholarship,
% \citet{thistlethwaite1960regression} had excluded
% from the sample the 13\% of finalists who had gone on to win National
% Merit scholarships.   Having done this, they also excluded \textsc{ancova},
% on the basis that such a biased sample was no basis for estimating
% \textsc{ancova} parameters.
% % Had the model been appropriate, the difference $\alpha_{1} - \alpha_{0}$
% % would have represented the treatment effect.
% They opted instead to customize a linear extrapolation of outcomes
% from the lower side of their cutpoint to the upper side, arguing that
% under the circumstances this extrapolation would better simulate
% competition finalists' outcomes under the hypothetical condition that
% a random selection of them had had their recognition as finalists
% withheld, despite having scored above the threshold. This formulation
% suggests they might have preferred a potential outcomes version of the
% \textsc{ancova} model:
A potential outcomes version of the model is
 $Y_{Ci} = \alpha_{0} + \beta R_{i} + \epsilon_{Ci}$,
 $\epsilon_{Ci} \sim \mathrm{Normal}(0, \sigma^{2})$; $D_{i} \leq
 Z_{i} = \indicator{R_{i} \geq 0}$; $Y_{Ti} = \alpha_{0} +
 (\alpha_{1} - \alpha_{0})D_{Ti} + \beta R_{i} + \epsilon_{Ti}$,
 $\epsilon_{Ti} \sim \mathrm{Normal}(0, \sigma^{2})$.
In marked contrast to the model of \S~\ref{sec:randProc}, it does
...
determines $Z$. Nonetheless, under this model the ordinary least squares
(OLS) contrast $\hat{\alpha}_{1} - \hat{\alpha}_{0}$ is unbiased for
$\alpha_{1} - \alpha_{0}$ --- the value of  $\lim_{r\downarrow 0} \EE(Y | R=r) -
\lim_{r\uparrow 0} \EE(Y | R=r)$ and, simultaneously,
limitless estimation targets such as $\EE Y_{T}  - \EE Y_{C}$.


To relate this to the formulation of \S~\ref{sec:randProc}, write
$\dt[(a, b)]{\mathbf{y}_{H}}{ \mathbf{r}} = {\mathbf{y}_H} - a -
b\mathbf{r}$.
Given $H: Y_{T} = Y_{C} + \tau D_{T}$ define ${{Y}_H} = {Y} - \tau {D}$ and
$\hat\theta = (\hat\alpha, \hat\beta)$, the intercept and $R$-slope
estimated through regression of ${\mathbf{Y}_H}$ on
$\mathbf{1}, \mathbf{R}$ and $\mathbf{Z}$. Then
$\dt[\hat\theta]{ {\mathbf{Y}_H}}{ \mathbf{R}}$ is the partial residual
ignoring contributions from $Z$. The statistic
$%t(\mathbf{Y}_H , \mathbf{Z}) =
\overline{\dt[\hat\theta]{{{Y}_H}}{ {R}}}_{Z=1} -
\overline{\dt[\hat\theta]{{{Y}_H}}{ {R}}}_{Z=0}$
may be used to test $H$.  Because of the structural relationship
between $R$ and $Z$, the distribution of this statistic as conditioned
on $(N_{0}, N_{1}, \mathbf{Y}_{C})$ is not amenable to tabulation, even under
hypotheses $H$ that correctly specify each value of $Y_{T} - Y_{C}$;
permutation tests
%of $H$ without need for modeling of the distribution of $(Y_{C},
%Y_{T})$, as in \eqref{eq:01},
are not
tractable.  Under the parametric \textsc{ancova} model, however, with
conditioning on $\mathbf{R}$ rather than on $(N_{0}, N_{1},
\mathbf{Y}_{C})$ (or \S~\ref{sec:randProc}'s $\mathbf{A}^{*}$), it is straightforwardly Normal. Indeed, it is
...
%% of $Z$; the main assumption of Sections~\ref{sec:rand}
%% and \ref{sec:covariates} is violated.  Can it be relaxed to an assumption that is
