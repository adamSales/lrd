...
<<modelMariaDeaths,include=FALSE,dependson='loadMariaDat'>>=
### model monthly deaths prior to 2017
mod  <-   lmrob(deaths_original~year+
...
dat$resid <- dat$deaths_original-dat$yhat

@

<<mariaFigs,include=FALSE,dependson='modelMariaDeaths'>>=
pd <- rbind(dat,dat)
pd$Deaths <- c(dat$deaths_original,dat$resid)
...
ggplot(pd,aes(month_num,Deaths,group=year,color=year,alpha=hurYear))+
  ...
  stat_function(data=data.frame(x=dat$month_num,type='Death Counts',y=dat$deaths_original,year='avg',hurYear=TRUE),
...
using a robust redescending M-estimation procedure, which systematically
...
Comparing
$t_{u}(\mathbf{\resid},\mathbf{Z})=$\Sexpr{signif(test[['statistic']],2)}
