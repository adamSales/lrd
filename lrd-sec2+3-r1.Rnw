...
of strictly no effect, $H_{0}: \resid_{T} \equiv \resid_{C}$, under which
...
%%%% unfinished, but gotta stop for now...
%%% plan: estimate maria p-value here

%, and
the hypothesis of a constant dose effect $\tau$, under which
$\bm{y}_{C} = \bm{y} - \tau \bm{d}$.
Given $H: \tau = \tau_{0} \in (-\infty, \infty)$, define $\resid_{H} = Z(\resid_{T} - \tau_{0}) + (1-Z)\resid_{C}$.
With such an $H$ under test,
   The constant dose effect model
situates $\PP(\resid_{T} - \resid_{C}| D, R)$ within a  translation family,
making $(N_{0}, N_{1}, \bm{\resid}_{C})$ S-ancillary and justifying the Fisherian conditioning strategy.
