...
pfun <- function(tau){
  ...
  pHigh <- (nmore+.5)/cardZ
  pLow <- (nless+.5)/cardZ
  c(nless=nless,nmore=nmore,p=2*min(pHigh,pLow))
}
...
$\bm{A}^*=\{\bm{\resid}_C=\bm{\resid},N_1=$\Sexpr{N1}$,N_0=$\Sexpr{N0}$\}$ and calculate the
sampling distribution of test statistic $\bar{\resid}_{Z=1}$ by calculating its
value for all $\binom{\Sexpr{N}}{\Sexpr{N1}}=\Sexpr{cardZ}$ possible permutations
of $\bm{Z}$.
...
With moderate or large samples, M-estimation (also called
``generalized estimating equations'' or ``
