...
dat$resid <- dat$adjDeath-dat$yhat

@

<<mariaFigs,include=FALSE,dependson='modelMariaDeaths'>>=
pd <- rbind(dat,dat)
pd$Deaths <- c(dat$adjDeath,dat$resid)
...
ggplot(pd,aes(month_num,Deaths,group=year,color=year,alpha=hurYear))+
  ...
  stat_function(data=data.frame(x=dat$month_num,type='Death Counts',y=dat$adjDeath,year='avg',hurYear=TRUE),
