...
dat <- read.csv('../death_file_JAMA.csv')
...
<<bandwidth,include=FALSE,eval=TRUE>>=
...
Hurricaine Maria struck the island of Puerto Rico on September 20,
2017.
...
In the course of our discussion, we will also review two classical
approaches to hypothesis testing (based largely on
\citealt{fisher:1935}) and estimation (based largely on
\citealt{huber1964robust,huber1967behavior}).
Though these ideas are old, some aspects of our framing will be
novel.
...
$ZY_{T}+(1-Z)Y_{C}$.
The treatment effect, the number of deaths Maria would cause, in
the $i$th month is $\tau_i=Y_{Ti}-Y_{Ci}$.

One approach to estimating the death toll from
Maria models $\bm{Y}$ and $\bm{Z}$ as if arising from a randomized
...
\end{equation}
%\end{ass}\
which is a simpler form of ``residual ignorability,'' which we define
more generally below (Section~\ref{sec:model-eey-c-r}).
...
Under residual ignorability
\eqref{eq:ignore2},  as in RCTs, the probability distribution of $Z$ is modeled
with far greater confidence than that of $\resid$, or of $(\resid_{C},
\resid_{T})$.
Continuing in the manner of \citet{fisher:1935},
we leverage this asymmetry by conditioning on
$\bm{\resid}_C\equiv\{\resid_{Ci}\}_{i=1}^{n}$,
rendering irrelevant what distribution they were drawn from.\footnote{\textit{Design of Experiments} [\citeyear[ch.~1]{fisher:1935}]
...
and $N_{0}$ of the treatment and control group samples, where
$N_{j} \equiv \sum_{i=1}^{n} \indicator{Z_{i}=j}$.
Such conditioning is appropriate because the conditioning statistic
$\bm{A}^*=\{\bm{\resid}_C,N_1,N_0\}$ is appropriately ancillary to, i.e. carries no information
about, the target of estimation, which is of the form
$\EE (\resid_{T}-\resid_{C} | \mathcal{E})$, some event $\mathcal{E}$,
because it does not constrain $\bm{\resid}_T$.
In contrast, $\bm{A}^*$ would not be ancillary to targets of the form $\EE(Z| \mathcal{E})$
or $\EE(\resid_{C}| \mathcal{E})$.
Formally, for a
statistical model situating $\PP(\resid_{T} - \resid_{C}|
R)$ in a translation family, $\bm{A}^*$ is S-ancillary
\citep{cox2006pos,lehmannRomano2006TSH} to the target parameters.

An oddity of conditioning on $\bm{A}^*$
is that it does not correspond to any simple reduction of observed
data, since $\resid_{Ci}$ is observed only if $Z_{i}=0$.
This approach is feasible when testing
...
more readily tested).
Then, $\bm{A}^*$ is temporarily assumed equal to $(N_{0}, N_{1}, \bm{\resid})$,
which is a function of observables and is thus a statistic in the ordinary
sense.
Under $H_0$, we may exactly enumerate the sampling distribution of any test statistic conditional on
$\bm{A}^*$; we estimate exact p-values by comparing a test statistic
to its conditional distribution thus enumerated.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% change this if we use a different window of analysis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In the Maria example, the null hypothesis $H_0$ means that hurricane
Maria caused precisely no change in each month's death count (and
hence each month's residual $\resid$).
Under $H_0$, we condition on
$\bm{A}^*=\{\bm{\resid}_C=\bm{\resid},N_1=3,N_0=3\}$ and calculate the
distribution of test statistic $\bar{\resid}_{Z=1}$ by calculating its
value for all $\binom{6}{3}$ possible permutations
of $\bm{Z}$.
It turns out that largest value of test statistic
$\bar{\resid{z=1}=$\Sexpr{with(subset(dat,hurYear&Z),round(mean(resid)))}
occurs when $\bm{Z}=\bm{z}$, i.e. when September, October, and
November are the affected months.
Therefore, the one-sided p-value testing $H_0$ is
$1/\binom{6}{3}=1/20$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\bm{\resid}_{C} = \bm{\resid}_H\equiv\bm{\resid} - \tau_0 \bm{z}$.
...
$H_{\tau_0}$ to a permutation test.  This confers an
...
\subsection{Estimating Effects and Standard Errors}\label{sec:rand}

\marginpar{Replace all this with M-estimation?}

<<estimates,include=FALSE>>=
ebar0 <- with(subset(dat,year==2017&month_num<9),round(mean(resid)))
ebar1 <- with(subset(dat,year==2017&month_num>8),round(mean(resid)))
est <- ebar1-ebar0
tot <- est*4

test <- t.test(resid~I(month_num<9),data=dat,subset=year==2017)
pval <- test$p.value
ci <- test$conf.int
ci4 <- ci*4
@
...
% $\bm{A}^{*}=\{N_{1},N_{0},\tilde{\bm{\resid}}_{T},\tilde{\bm{\resid}}_{C}\}$,
% where  $\tilde{\bm{\resid}}_{T} = \bm{\resid}_{T} - \bar{\resid}_{T}$ and
% $\tilde{\bm{\resid}}_C$ are centered potential outcomes.
...
% As a result, $\tilde{\bm{\resid}}_{T}$ and $\tilde{\bm{\resid}}_{C}$ are
...
% on $\bm{A}^{*}$ allows for randomization- (if not permutation-)
% based inference.

Consider the archetypical
Neyman-type test statistic \citeyearpar{neyman:1923,neyman1934stratifiedsampling}, %is
  % If $\tilde{\bm{y}}_H$ denotes the potential response
  % $\bm{y}_{c}$ that would be entailed by a strong null $H$, in
  % combination with observations $\bm{y}$ and $\bm{d}$, then
...
\mathrm{SE}_{u}^2 \left( \bar{\resid}_{Z=1} -  \bar{\resid}_{Z=0} \right) &= \frac{s_{z=1}^{2}({\bm{\resid}})}{n_{1}} +
    \frac{s_{z=0}^{2}({\bm{\resid}})}{n_{0}}  \label{eq:sudef}
% \\
%     s_{z=j}^{2}(\bm{v}) &= \frac{1}{n_{j}-1}\sum_{i: z_{i} = j}
% (v_{i} - \bar{v}_{z=j})^{2} .\nonumber
% \end{equation}
\end{align}
is the ordinary unpooled two-sample variance. With conditioning on
$\bm{A}^{*}$, and assuming \eqref{eq:ignore2}, the expected value
of \eqref{eq:sudef} is
${s^{2}({\bm{\resid}_{T}})}/{N_{1}} +
{s^{2}({\bm{\resid}_{C}})}/{N_{0}}$, an upper bound of the variance of
...
$t_{u}(\bm{\resid}, \bm{Z})\stackrel{d}{\rightarrow}\mathrm{Normal}(0,v)$,
some $v\leq 1$, conditional on $\bm{A}^{*}$; see
Appendix~\ref{apnd:RICLT}.
More generally, for any $\tau$ we can assess
$t_{u}(\bm{\resid}-\tau\bm{Z}, \bm{Z})$ against a standard Normal
...
The value $\hat{\tau}_{\text{HL}}$ for which $t_{u}(\bm{\resid}-\hat{\tau}_{\text{HL}}\bm{Z},
\bm{Z})$ is equal to its null expected value of 0 is a Hodges-Lehmann estimate of the true average effect\footnote{The general definition of
...
was roughly
$\bar{\resid}_{Z=0}=$\Sexpr{ebar0} deaths;
in the final four months, we have
$\bar{\resid}_{Z=1}=$\Sexpr{ebar1}
deaths, implying an estimated death toll of \Sexpr{est} deaths
per month, or a total of \Sexpr{tot} deaths.
Comparing
$t_{u}(\bm{\resid},\bm{Z})=$\Sexpr{signif(test[['statistic']],2)}
to a t-distribution with \Sexpr{signif(test[['parameter']],2)} degrees
of freedom gives a two-sided p-value of \Sexpr{signif(test[['p.value']],2)}
testing the null hypothesis of no excess deaths
average.
Inverting the t-test gives a 95\% confidence interval of
[\Sexpr{ceiling(ci[1])}, \Sexpr{floor(ci[2])}] excess deaths per
month, or a total death toll of
[\Sexpr{ceiling(ci4[1])}, \Sexpr{floor(ci4[2])}].
...
comparing $t_{u}(\bm{\resid}-\tau\bm{D}, \bm{Z})$ to the
...
$\resid$-ignorability assumption and the analysis steps that build on
it.
...
residual transformations, each mapping data $(\bm{y}, \bm{r})$ to
$\{\dt {y_{i}}{\bm{r}} \}_{i=1}^{n}$.
...
$\bm{A}= (\dt[\thetaInf]{\bm{Y}_{C}}{ \bm{R}}$, $\bm{D}_{T})$.
Conditioning on
$\dt[\thetaInf]{\bm{Y}_{C}}{ \bm{R}}$
removes little of the entropy
in $\bm{R}$, leaving it available as a basis for inference.
Uncoupled to $Y_{T}$s, the detrended  $Y_{C}$s,
$\dt[\thetaInf]{\bm{Y}_{C}}{ \bm{R}}$,
are in themselves uninformative about $\EE(Y_{T} - Y_{C})$, so
the variables comprising $\bm{A}$ are jointly
%\marginpar{$\leftarrow \Delta$ to $(\bm{Y}_{C}, N_{0}, N_{1})$?}
S-ancillary, just as $\bm{A}^{*}$ was seen to be
...
$\dt[\hat\theta]{\bm{y}_{H}}{\bm{r}}$, completing the
...
Iterative testing is facilitated by regressing $\bm{y}$ on $\bm{r}$ and $\bm{z}$
with offset variable $\bm{d}\tau_{0}$; then only the offset needs to
...
a $O(n^{-1/2})$-sized
share of the sample --- consistency for $\thetaInf$ requires
a redescending M-estimator, or
robust regression with bounded psi function (\citealp[Thm.~3]{he1991localbreakdown};
...
proper limits of $\mathcal{W}$, robust M-estimation (\S~\ref{sec:robust-altern-ordin}) is to be preferred.
...
plots of Cook's distances
 ...
 as to possess a bounded psi function
...
independence of
 $\bm{Z}$ from $(\bm{X}, \bm{Y}_{C}, \bm{Y}_{T})$.
%% Analogous statements will hold for more complex randomization or
%% ignorability assumptions as well.
 Although neither $\bm{Z} \independent \bm{Y}_{C}$ nor
 $\bm{Z} \independent \bm{Y}_{T}$ can be directly tested,
 since potential outcomes are only partly observed, assumptions of form
 $\bm{Z} \independent \bm{X}$ are falsifiable: researchers can
 conduct placebo tests for effects of $Z$ on $X$.
% Just as $H: \bm{y}_{T} - \bm{y}_{C} =
% \bm{\tau}$ is tested in the strong randomization inference
...
% the covariate, $\overline{\dt[\thetaInf]{\bm{x}}{\bm{r}}}_{Z=1} - \overline{\dt[\thetaInf]{\bm{x}}{\bm{r}}}_{Z=0}$,
...
%\citealp[][Ch.~14]{lehmannRomano2006TSH}
 ...
 for marginal associations of $\bm{Z}$ with covariates $\bm{X}_{i}$,
...
%  $t(\bm{x}_{i}, \bm{z}) = |\overline{(x_{i})}_{z=1} -
%  \overline{(x_{i})}_{z=0}|$,
%  $ i \leq k$, after conditioning on $\bm{X}$ in addition to
%  $\sum_{i} Z_{i}$.
Relatedly, \citet{lee2010regression} recommend a
 tests for conditional association, given $R$, of $\bm{Z}$ and
 $\bm{X}$, by fitting models like those discussed in
...
%estimation.
This approach addresses multiplicity of placebo tests as a matter of course; as with the method of \citet{cattaneo2014randomization}, however, its implied placebo tests, of whether $Z$ ``affects'' $\bm{X}$, are not covariance adjusted for $R$.
...
$\bm{Y}_{C}^{*} = (\bm{X}, {Y}_C)$ in place of $Y_{C}$.
...
vis a vis $\bm{R}$, just as ${\bm{Y}_C}$ will later be decomposed.
