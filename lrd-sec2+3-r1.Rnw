...
sampling distribution of test statistic $T(\bm{\resid},\bm{Z})=\bar{\resid}_{Z=1}$ by calculating its
...
In the Maria example, this approach estimates an effect of hurricaine
...
M-estimation \citep[][also called ``generalized estimationg
