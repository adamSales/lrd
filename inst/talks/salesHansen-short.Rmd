...
- Bounded influence robust regression substitutes out OLS' $e_{(a,b)}(y|r) = y-a-br$ with $e_{(a,b,s)}(y|r) = \psi\{(y - a -br)/s\}$, for a carefully chosen $\psi(\cdot)$ and carefully estimated $s$.
- Generate "robustness" weights $\in [0,1]$, and additional diagnostic.
- Asymptotically, only these methods limit sensitivity to contamination of a $O(n^{-1/2})$ share of the sample.
- Examples: "MM with bisquare psi-function and robust scale",...
- Far trickier than "robust standard errors".  
- In R, `robustbase::lmrob`; in Stata, `mmregress`.
- (This method and $\mathcal{W}$ give similar estimates to LSO's.)
