...
## Academic probation and later grades | Data from 3 Canadian colleges [@lindo2010ability] {.build}
...
> - $H'$ is sustainable with wider $\mathcal{W}$ than $H$. (Don't throw away good data.)
...
>  - Canadian college data have a clump of narrow avoiders of acad. probation at $R=0$.   
> - To avoid rejecting $\tilde{H}$, $\mathcal{W} \mapsto \mathcal{W}\setminus \{0\}$ and $\tilde{\mathcal{W}} \mapsto \tilde{\mathcal{W}}\setminus \{0\}$. (Carve hole in middle, leaving "donut.")
...
Say there are "savvy" students who, when at risk for AP, "manipulate" grades $\uparrow$. (Bribes?)  This latent subgroup is overrepresented in excised donut hole $\{i: z_i=0\}$, but may be elsewhere also --- as many as $O(n^{1/2})$ "bad" points remain, undercutting estimation of $E(Y_C | R)$. 
...
It requires us to abandon OLS, which is hypersensitive to contamination. In contrast, robust MM estimates are insensitive enough to prevent $\mathrm{Bias}(\hat\tau)$ from dominating $\mathrm{MSE}(\hat\tau)$. 
