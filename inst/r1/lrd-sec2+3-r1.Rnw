...
Outliers form an additional threat; 
\marginpar{Postpone? \ldots} % In this context robustness risks distracting
% from main ideas.  Might be helpful to the reader to save up the robustness
% ideas (2014 outliers, contamination more broadly) for a later section the 
% paper. -B
...
This may be summarized in a new ignorability assumption:
...
In the Maria example, the null hypothesis $H_0$ states that hurricane
...
$\bm{A}^*=\{(Y_{-71}, R_{-71}), \ldots, (Y_{-1}, R_{-1}); \bm{\resid}_C=\bm{\resid},N_1=$\Sexpr{N1}$,N_0=$\Sexpr{N0}$\}$ and calculate the
