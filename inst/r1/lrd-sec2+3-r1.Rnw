...
$ZY_{T}+(1-Z)Y_{C}$.
The difference $\tau_i=Y_{Ti}-Y_{Ci}$, for each of months $i=9,
\ldots, 12$, is the excess mortality caused by Maria.

One approach to estimating the death toll from
\marginpar{Consider postponing/removing, to maintain focus on our idea.}
...
months---particularly December and January---than during the rest of
the year.

Dependence between $R$ and $Y_C$---and hence violations of
\marginpar{Consider postponing/removing this \P also}
...
October---were unusually high.\footnote{We are unaware of weather
  or other events explaining the late 2014 mortality spike. Official 
