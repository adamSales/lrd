...
In the case of imperfect compliance, let $D$ indicate whether treatment was actually
received.
...
With
moderate or large samples, the approach deriving from Neyman
\citeyearpar{neyman:1935} gives randomization- but not
permutation-based inferences, enabling it to test hypotheses of form $H: \EE(Y_T - Y_C) =c$, as opposed to the stronger hypotheses of form $H: Y_T - Y_C \equiv c$ tested in Section~\ref{sec:using-eqref-test}. 
...
\mathbf{Y}_{C})$ (or \S~\ref{sec:randProc}'s $\mathbf{A}^{\dagger}$), it is straightforwardly Normal. Indeed, it is
