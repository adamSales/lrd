...
\end{equation}
where $\hat{\alpha}$ and $\hat{\beta}$ are the OLS estimates of
intercept and slope in a variant of \eqref{eq:classicOLS}
with $\mathbf{y}_{H}$ replacing $\mathbf{y}$ as dependent variable. Thus
$\meanDiffT(\mathbf{y}_{H}, \mathbf{z})$ coincides with the $\tau$-estimate
from the same OLS fit. 
...
because trend parameters had been estimated
from a separate dataset.) However, under the parametric \textsc{ancova} model, with
...
\subsubsection{Fuzzy Regression Discontinuity} \label{sec:fuzzy-regr-disc}
...
process remains unchanged [\citealp{rosenbaum:1996:onAIR}].)
