...
`robustbase::.vcov.avar1`.   However, during initial fitting we'll
stick with covariance estimation via `robustbase:::.vcov.w`, which
seems less likely to throw errors. 

```{r}

controlsettings <- list()
controlsettings$SMdefaults <- lmrob.control(cov = '.vcov.w')
controlsettings$ks2011like <- lmrob.control(method = 'SM', 
psi = 'lqq', max.it = 500,
     k.max = 2000, cov = '.vcov.w')
        ...
        for (infn in names(initfunctions))
            {
                sims[[paste0(ns, ctls, infn, collapse=":")]] <- 
                    replicate(nreps, {
                        ...
                        fit$control$method <- "SM"
                        newcov <- try(robustbase:::.vcov.avar1(fit), silent=TRUE)
                        if (!inherits(newcov, "try-error")) fit$cov <- newcov
