...
\newcommand{\InfPar}[1]{\ensuremath{{#1}_{0}}}
...
\begin{itemize}
\item ``In 3.1, the notation $\dt[\theta_{\infty}]{Y_{C}}{ R }$ is unclear.''
\begin{itemize}
  \item The symbol ``$\theta_{\infty}$'' has been replaced with
    ``$\theta_{0}$,'' and text introducing $\thetaInf$ (following display
    ...
    within this notation is introduced earlier in the paper, 
     in \S~\ref{sec:maria} using a simpler notation ($E_{C}$) 
