...
This estimation procedure, like others based on inversion of a
hypothesis test, may appear to require an assumption of a constant
additive treatment effect, such as $Y_T=Y_C+\tau_0$ or
$Y_T=Y_C+D\tau_0$, for some constant $\tau_0$.
However, the use of robust standard error estimates weakens this assumption.
Under \eqref{ycheck}, $\hat\tau$ is consistent for $\bar\tau$ solving
\begin{equation} \label{eq:concl1}
\EE \big\{\dt[\thetaInf]{\mathbf{Y}_{H}}{ R } \big| R \big\}
\equiv \EE \big\{ \dt[\thetaInf]{Y_{C}}{ R } \big| R \big\},
\end{equation}
providing such a $\bar\tau$ exists.
That is, there must be a hypothesis $H$ such that detrended $Y_H$ are
equal to detrended $Y_C$ on average, not exactly.
This flexibility is borne out in the simulation study in
\S~\ref{sec:levelPower}.
